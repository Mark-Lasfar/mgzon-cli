// /workspaces/mgzon-cli/src/commands/init.ts
...
export async function initCommand(projectName: string, options: any) {
  ...
  try {
    ...
    if (await fs.pathExists(projectDir) && (await fs.readdir(projectDir)).length > 0) {
      ...
      console.log(chalk.red('❌ Directory not empty!'));
      console.log(chalk.red('─'.repeat(50)));
      console.log(chalk.cyan(`   Directory: ${projectDir}`));
      console.log(chalk.cyan('   Choose a different name or empty the directory\n'));
    ...
    console.log(chalk.gray(`   Debug: Looking for template: ${templatePath}`));
    
    if (await fs.pathExists(templatePath)) {
      console.log(chalk.gray(`   Debug: Found template, copying...`));
      await fs.copy(templatePath, projectDir);
    } else {
      console.log(chalk.gray(`   Debug: Template not found, creating basic structure...`));
      // Create basic structure
      await createBasicStructure(projectDir, options, apiUrl);
    }
    
    // إضافة ملف config خاص بالمشروع
    ...
    console.log(chalk.gray(`   Config file: ${path.join(projectDir, '.mgzon.json')}`));
    ...
    if (error.code === 'ENOENT') {
      ...
      console.log(chalk.cyan('   Make sure you have write permissions to the directory'));
    }
  }
}

async function createBasicStructure(projectDir: string, options: any, apiUrl: string) {
...
async function createProjectConfig(projectDir: string, apiUrl: string) {
  const config = {
    apiUrl,
  ...
  await fs.writeJson(
    path.join(projectDir, '.mgzon.json'),
    config,
    { spaces: 2 }
  );
  
  console.log(chalk.gray(`   Created: .mgzon.json`));
}